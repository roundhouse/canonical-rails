module CanonicalRails
  module TagHelper
    ...
    def canonical_href(host=canonical_host, path=nil, protocol=canonical_protocol)
      raw "#{protocol}#{host}#{path || path_without_html_extension}#{trailing_slash_if_needed}#{whitelisted_query_string}"
    end

    def canonical_tag(host=nil, path=nil)
      host ||= canonical_host
      tag(:link, :href => canonical_href(host, path), :rel => 'canonical')
    end
